...
class LemmaNonStandardRepositoryImpl implements LemmaNonStandardRepository
{
     ...
     * при сохраненнии лемм, которые уже существуют в БД обновляется значение частоты, с которой лемма встречается на странице
     * @param lemmaStrings коллекция, содержащая значения лемм (String)
    ...
    private List<Lemma> findSaveUpdateLemmasInDB(Collection<String> lemmaStrings, int siteId)
    {
        ...
        String qryPrefixUpdate = "INSERT INTO " + tablesPrefix + "lemma (lemma, site_id, frequency) VALUES (";
    ...
    /**
     * олучение сочетания значение леммы - id леммы из списка объектов лемм
